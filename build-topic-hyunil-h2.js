...
const NAV = {
  ...
  extraLinks: [
    ...
    { href: '2026년-1학기말고사-현일고1-주제-모음.html', label: '📌 현일고1 주제' },
  ],
};
...
function collectH2Series() {
  ...
  const mockItems = parseMainLinkCollection('collections/현일고2-2026-1학기말-5월이투스.html', { dedupe: true });
  ...
  const mockItemsEnriched = enrichItems(mockItems, '2026 5월 이투스 모의고사');
  if (mockItemsEnriched.length) {
    allSeries.push({
      title: '2026 5월 이투스 모의고사',
      items: mockItemsEnriched,
      badgeFn: (it) => it.code,
    });
  }
...
const result = runTopicBuild({
  ...
  heroTheme:
    '현일고2 1학기말 시험범위 <strong>해커스 수능독해 불변의법칙(1·2·제1회)</strong>과 <strong>2026 5월 이투스 모의고사</strong> 지문의 <strong>주제(요지)</strong>를 한글로 정리했습니다. <strong>주제·요지·제목 고르기</strong> 대비용으로 암기하세요.',
  heroSentence:
...
console.log('총 지문:', result.totalPassages, `(부교재 ${result.primaryCount} + 5월 이투스 ${result.secondaryCount})`);
